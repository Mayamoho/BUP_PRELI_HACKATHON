...
def interpret_notes_llm(notes: list[str], capacity: float, feedback: str | None = None, budget_seconds: float | None = None) -> list[dict]:
    ...
    for _round in range(3):
        limited = False
        benched = [t for t in targets if _cooling(t)[0] > 0 and not _cooling(t)[1]]
        if len(benched) == len(targets):
            benched = []  # every target recently failed: try them all again rather than fail unasked
        for target in targets:
            wait, was_limited = _cooling(target)
            if wait > 0 and (was_limited or target in benched):
                if was_limited:
                    limited = True
