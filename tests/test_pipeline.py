...
def test_end_to_end_without_llm(monkeypatch):
    ...
    for case in CASES:
        ...
        assert replay(case["input"], body, case["expected_output"]["directive_interpretation"]) == []
...
def test_llm_bad_output_is_contained(monkeypatch):
    ...
    assert r.status_code == 200
    assert r.json()["directive_interpretation"][0]["directive_type"] in llm.SYSTEM_PROMPT
